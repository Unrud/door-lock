...
def verify_totp(time_, secret, pin):
    for deviation in range(
        -config.TOTP_ACCEPTED_DEVIATION, config.TOTP_ACCEPTED_DEVIATION + 1
    ):
        if (
            pin
            == totp(
                time_ + deviation * config.TOTP_STEP_SECS,
                secret,
                config.TOTP_STEP_SECS,
                config.TOTP_DIGITS,
            )[0]
        ):
            return True
...
def index(request):
    ...
    return Response(body=stream(), headers={"Content-Type": "text/html; charset=utf-8"})
...
def auth(id_, password):
    ...
    if len(password) != config.TOTP_DIGITS or not verify_totp(time_, secret, password):
...
def status(request, status, status_text):
    ...
    if (
        status < 200
        or (300 <= status and status < 400)
        or 600 <= status
        or len(status_text) > 100
        or not re.match(r"^[A-Za-z]+(?: [A-Za-z]+)*$", status_text)
    ):
